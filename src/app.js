...
function addChocolates(chocolates, color, count) {
  ...
  : helpAdd(chocolates, color, count);
}
...
function dispenseChocolates(chocolates, number) {
  ...
  return number <= 0
  ? "Number cannot be zero/negative"
  : lengthOfChocolatesStore < number? "Insufficient chocolates in the dispenser"
...
const helpNoOfChocolates = function (chocolates) {
  const answer = [];
  ...
  referenceChocolates.forEach((refChocolate) => {
    ...
    counter > 0 ? answer.push(counter) : null;
  });
  return answer;
};
function noOfChocolates(chocolates) {
  return helpNoOfChocolates(chocolates);
}

//Progression 6: Sort chocolates based on count in each color. Return array of colors
const helpSortChocolates = function(chocolates) {
  let answer = [];
  let store = {};
  chocolates.sort();
...
function sortChocolateBasedOnCount (chocolates) {
  finalAnswer = helpSortChocolates(chocolates);
  return finalAnswer;
}

//Progression 7: Change z chocolates of x color to y color
const helpToChangeChocolateColor = function (chocolates, number, color, finalColor) {
  for (let i = 0; i < chocolates.length; i++) {
...
function changeChocolateColor(chocolates, number, color, finalColor) {
  ...
  : helpToChangeChocolateColor(chocolates, number, color, finalColor);
}

//Progression 8: Change all chocolates of x color to y color and return [countOfChangedColor, chocolates]
function changeChocolateColorAllOfxCount(chocolates, color, finalColor){
  const changedColor  = helpToChangeChocolateColor(chocolates, chocolates.length, color, finalColor);
  counter = 0;
